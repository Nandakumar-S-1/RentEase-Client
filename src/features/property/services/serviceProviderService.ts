...
export const addServiceProvider = async (
  data: Omit<
    ServiceProvider,
    "id" | "rating" | "totalJobsCompleted" | "isActive"
  >,
) => {
  const response = await axiosApi.post<ApiResponse<ServiceProvider>>(
    BASE_URL,
    data,
  );
...
export const toggleServiceProviderStatus = async (
  id: string,
  isActive: boolean,
) => {
  const response = await axiosApi.patch<ApiResponse<null>>(
    `${BASE_URL}/${id}/status`,
    { isActive },
  );
  return response.data;
...
export const deleteServiceProvider = async (id: string) => {
  const response = await axiosApi.delete<ApiResponse<null>>(
    `${BASE_URL}/${id}`,
  );
  return response.data;
};
...
export const updateServiceProvider = async (
  ...
  const response = await axiosApi.put<ApiResponse<ServiceProvider>>(
    `${BASE_URL}/${id}`,
    data,
  );
  return response.data;

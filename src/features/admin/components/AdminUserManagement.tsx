/**
 * Admin User Management - Owners & Tenants Separated
...
const AdminUserManagement = () => {
    ...
    const fetchUsers = async (page = 1, role: 'OWNER' | 'TENANT') => {
        try {
            setLoading(true);
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                                    ...
                                    <tr key={user.id} className="hover:bg-gray-50/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4 text-center">
                                            <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-lg text-[10px] font-bold ${getStatusStyles(user)}`}>
                                                <div className={`w-1 h-1 rounded-full ${getStatusDot(user)}`}></div>
                                                {getStatusLabel(user)}
                                            </span>
                                        </td>
                ...
                <div className="p-6 bg-gray-50/50 flex items-center justify-between border-t border-gray-100">
                    <p className="text-xs text-gray-400">
                        Showing {filteredUsers.length} of {pagination.total} {userType.toLowerCase()}
                    </p>

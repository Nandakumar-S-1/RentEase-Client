...
import { getPendingOwners, verifyOwner, rejectOwner, type PendingOwner } from "../services/adminVerificationService";
...
export const useOwnerVerification = () => {
    ...
    const fetchPendingOwners = useCallback(async () => {
        try {
            ...
            setError(apiError?.response?.data?.message || "Failed to fetch pending owners");
        } finally {
            setIsLoading(false);
        }
    }, []);
    ...
    return { owners, isLoading, error, successMessage, fetchPendingOwners, approve, reject };
};

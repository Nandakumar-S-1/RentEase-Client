...
export const signTenant = async (
  id: string,
  signatureUrl: string,
): Promise<{ pdfUrl: string }> => {
  const response = await axiosApi.post(API_ROUTES.SIGN_TENANT(id), {
    signatureUrl,
  });
  return response.data.data;
};
...
export const getUploadUrls = async (
  ...
  const response = await axiosApi.post(
    API_ROUTES.GET_AGREEMENT_UPLOAD_URLS(id),
    { files },
  );
  return response.data.data;
};

...
export const useAgreements = () => {
    ...
    const createAgreement = useCallback(async (data: Partial<Agreement>) => {
        try {
            ...
            setError(apiError?.response?.data?.message || "Failed to create agreement");
            throw err;
        } finally {
            setIsLoading(false);
        }
    ...
    const getAgreementById = useCallback(async (id: string) => {
        try {
            ...
            const apiError = err as ApiError;
            setError(apiError?.response?.data?.message || "Failed to fetch agreement");
            throw err;
        } finally {
            setIsLoading(false);
        }
    ...
    const getMyAgreements = useCallback(async () => {
        try {
            ...
            const apiError = err as ApiError;
            setError(apiError?.response?.data?.message || "Failed to fetch your agreements");
            throw err;
        } finally {
            setIsLoading(false);
        }
    ...
    const uploadKycFile = useCallback(async (id: string, file: File) => {
        try {
            ...
            setError(apiError?.response?.data?.message || error.message || "Failed to upload KYC document");
            throw err;
        } finally {
            setIsLoading(false);
        }
    ...
    const uploadSignatureFile = useCallback(async (id: string, file: File): Promise<string> => {
        try {
            setIsLoading(true);
            setError(null);
            ...
            const apiError = err as ApiError;
            const error = err as Error;
            setError(apiError?.response?.data?.message || error.message || "Failed to upload signature file");
            throw err;
        } finally {
            setIsLoading(false);
        }
